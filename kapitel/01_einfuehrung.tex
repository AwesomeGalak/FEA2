...
Erst 2024 nahm xAi die Firma von Elon Musk ein Cluster in Betrieb, welches über 100.000 NVIDIA HGX H100 GPUs verfügt. Eine dieser GPUs entspricht der Das in Tennessee befindliche Cluster stellt den bislang größten KI-Supercomputer der Welt dar und dient dem Training des generativen Modells Grok sowie zur Bereitstellung der Rechenkapazitäten zum Betrieb dessen. \cite{brendel2025colossus}. 
Nicht nur xAi sondern auch Meta Google OpenAI haben ähnlichen Zugang zu solch großen Cluster.
...
Laut einer Prognose der Internationalen Energieagentur (IEA) könnte sich der weltweite Stromverbrauch von Rechenzentren bis 2030 verdoppeln, wobei der Bedarf an GPU-Cluster wie sie für KI-Workloads eingesetzt werden im Zeitraum 2025 bis 2030 um 225 \% steigt~\cite{janson2025rechenzentren}.
...
Der rasante Ausbau cloudbasierter KI-Dienste wirft wie bereits erwähnt, neben technischen und energetischen Fragen zunehmend auch Bedenken hinsichtlich \Datenhoheit, Sicherheit und technologischer Unabhängigkeit auf.
...
europäische Konkurenz im Bereich der führenden Sprachmodelle gibt es kaum.
...
Wie im vorherigen Kapitel bereits aufgegriffen, beschäftigt sich diese Arbeit primär mit der Verwendung von Large Language Models auf Edge-Geräten und On-Premise Anwendung. Deshalb wird in diesem Kapitel zuerst der aktuelle Entiwcklungsstand der führenden LLM-Modelle dargelegt um den Unterschied zwischen diesen und kleinen, spezialisierten Modellen zu verdeutlichen.
...
Einen entscheidenden Durchbruch markierte das Jahr 2017, als ein Team aus Ingenieuren um Vaswani von OpenAi die sogenannte Transformer-Architektur vorstellte.
...
Auf dieser Grundlage entstanden Modelle, die in der Lage sind, komplexe Muster in umfangreichen Textdaten zu erkennen und natürlichsprachliche Ausgaben zu erzeugen, sogenannte \textit{Large Language Models} (LLMs).
Laut Kasneci ist ein LLM ein künstliches Intelligenzsystem, das auf einer riesigen Menge von Textdaten trainiert wurde, um natürliche Sprache zu interpretieren und menschenähnliche Antworten auf textbasierte Eingabeaufforderungen oder Fragen zu generieren~\cite{llm_definition}.
...
Modelle, die mit maximal 48GB VRAM auskommen, lassen sich bereits auf einer einzelnen ConsumerGPU betreiben und sind somit für On Premise Szenarien besonders interessant.
Prinzipiell sind die jeweils neuesten Modellgenerationen die leistungsstärksten; zum Zeitpunkt der Erstellung dieser Arbeit stellt Qwen 3.5 von Alibaba mit einem relativen Arena-Elo-Score von 1450 das leistungsstärkste quelloffene Modell dar.
